...
<script>
	function searchBbs() {
		...
		if ($('#SearchChoice').val() == "bName") {
			...
			document.searchForm.action = "announce_list.do";
		}
		...
		<table class="table table-hover table-sm" width="500" cellpadding="0" cellspacing="0" border="1">
			...
			<c:forEach items="${list1}" var="dto">
				<tr>
					...
					<td scope="row">${dto.bName}</td>
					<td scope="row"><c:forEach begin="1" end="${dto.bIndent}">-</c:forEach> <a
						href="content_view.do?bId=${dto.bId}">${dto.bTitle}</a></td>
					<td scope="row">${dto.bDate}</td>
					<td scope="row">${dto.bHit}</td>
				</tr>
			</c:forEach>
			...
			<tr>
				<td colspan="6">
					...
					[&lt;&lt;]
						...
						<c:otherwise>
							<a href="announce_list.do?page=1">[&lt;&lt;]</a>
						</c:otherwise>
					...
					[&lt;]
						...
						<c:otherwise>
							<a href="announce_list.do?page=${page.curPage -1 }">[&lt;]</a>
						</c:otherwise>
						...
						[${fEach}] &nbsp;
							...
							<c:otherwise>
								<a href="announce_list.do?page=${fEach}">[${fEach}]</a> &nbsp;
					</c:otherwise>
						</c:choose>
					...
					[&gt;]
						...
						<c:otherwise>
							<a href="announce_list.do?page=${page.totalPage}">[&gt;&gt;]</a>
						</c:otherwise>
					</c:choose>
			</tr>
		</table>
