...
<script>
	function searchBbs() {
		...
		if ($('#SearchChoice').val() == "bName") {
			...
			document.searchForm.action = "archive_list.do";
		}
		...
		<c:forEach items="${list3}" var="dto">
			<tr>
				...
				<c:forEach begin="1" end="${dto.bIndent}">-</c:forEach> <a
					href="content_view3.do?bId=${dto.bId}">${dto.bTitle}</a></td>
				<td>${dto.bDate}</td>
				<td>${dto.bHit}</td>
				<td>${dto.bLike}</td>
			</tr>
			...
			<tr>
			<td colspan="6"><a href="write_view3.do">글작성</a></td>
			</tr>
			<tr>
			<td colspan="6">
				<!-- 처음 --> 
				...
				<c:otherwise>
					<a href="archive_list.do?page=1">[&lt;&lt;]</a>
				</c:otherwise>
				...
				<c:otherwise>
					<a href="archive_list.do?page=${page.curPage -1 }">[&lt;]</a>
				</c:otherwise>
				</c:choose> 
				...
				<c:forEach var="fEach" begin="${page.startPage}" end="${page.endPage}" step="1">
					...
					<c:otherwise>
							<a href="archive_list.do?page=${fEach}">[${fEach}]</a> &nbsp;
					</c:otherwise>
					</c:choose>
				</c:forEach> 
				...
				<c:choose>
					...
					<c:otherwise>
						<a href="archive_list.do?page=${page.totalPage}">[&gt;&gt;]</a>
					</c:otherwise>
				</c:choose>
		</tr>
	</table>
